...
def test_agentic_loop_round1_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result["agentic_trace"]["planner_plan"]["strategy"] == "single_query"
    assert result["agentic_trace"]["plan_final"]["strategy"] == "single_query"
    assert len(result["agentic_trace"]["rounds"]) == 1
    assert result["agentic_trace"]["rounds"][0]["reactor"]["cypher"].startswith("MATCH")
    assert result["agentic_trace"]["rounds"][0]["critic"]["verdict"] == "accept"
    assert result["agentic_trace"]["rounds"][0]["verdict"] == "accept"
    assert result["agentic_trace"]["rounds"][0]["deterministic_issues"] == []


def test_agentic_loop_replans_then_succeeds(monkeypatch: pytest.MonkeyPatch) -> None:
    # ─── Arrange：準備測試輸入、替身與前置狀態 ─────────────────────
    # ─── Act：呼叫被測流程，收集實際輸出與副作用 ─────────────────
    # ─── Assert：驗證關鍵結果，確保行為契約不回歸 ─────────────────
    monkeypatch.setattr(nl2cypher, "_run_planner_agent", lambda **_kwargs: _plan())
    ...
    monkeypatch.setattr(nl2cypher, "_run_reactor_agent", lambda **_kwargs: next(reactor_outputs))
    monkeypatch.setattr(
        nl2cypher,
        "execute_cypher",
    ...
    assert len(result["agentic_trace"]["rounds"]) == 2
    assert result["agentic_trace"]["rounds"][0]["verdict"] == "replan"
    assert result["agentic_trace"]["rounds"][0]["replan"]["strategy"] == "single_query"
    assert result["agentic_trace"]["rounds"][1]["verdict"] == "accept"
...
def test_agentic_loop_fail_fast_on_repeated_cypher(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(RuntimeError, match=r"Cypher generation failed after retries: repeated cypher"):
...
def test_agentic_loop_exhausted_raises_runtime_error(monkeypatch: pytest.MonkeyPatch) -> None:
    # ─── Arrange：準備測試輸入、替身與前置狀態 ─────────────────────
    # ─── Act：呼叫被測流程，收集實際輸出與副作用 ─────────────────
    # ─── Assert：驗證關鍵結果，確保行為契約不回歸 ─────────────────
    ...
    with pytest.raises(RuntimeError, match=r"Cypher generation failed after retries: retry"):
        nl2cypher._run_agentic_query_loop(
