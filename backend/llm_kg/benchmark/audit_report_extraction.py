...
from backend.llm_kg.kg_builder import (
...
def _default_report_path() -> Path:
    """執行 `_default_report_path` 的內部輔助流程。
...
def _build_output_dir() -> Path:
    """執行 `_build_output_dir` 的內部輔助流程。
    此函式封裝局部邏輯以提升可讀性，並維持既有輸入輸出與邊界行為。
    """
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    output_dir = Path("/Users/silver/Documents/鴻海/backend/llm_kg/benchmark/runs") / timestamp
