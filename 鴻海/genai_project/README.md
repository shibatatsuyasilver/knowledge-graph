...
- `llm_kg/llm_client.py`: shared provider-aware LLM client (`openai` / `ollama`)
- `llm_kg/kg_builder.py`: entity/relation extraction + Neo4j upsert
- `llm_kg/nl2cypher.py`: schema-grounded Text-to-Cypher generation
- `llm_kg/llm_deploy.py`: FastAPI wrapper (`/api/chat`, `/health`)
- `llm_kg/run_e2e_mlx_local.sh`: local one-command E2E runner (Neo4j + MLX + QA)
...
```bash
cd /Users/silver/Documents/鴻海/genai_project/llm_kg
export LLM_PROVIDER=openai
export OPENAI_BASE_URL=http://localhost:8080/v1
export LLM_MODEL=mlx-community/Qwen3-8B-4bit-DWQ-053125
uv run uvicorn llm_deploy:app --host 0.0.0.0 --port 8000
```
...
`genai_project/docker-compose.llmkg.yml` no longer requires an Ollama service by default.
...
- `genai_project/llm_kg/benchmark/configs/benchmark_zh_tw_100.yaml`
...
- `genai_project/llm_kg/benchmark/datasets/kgqa_zh_tw_100_v1.jsonl`
...
## Report extraction audit (finance-aware)
